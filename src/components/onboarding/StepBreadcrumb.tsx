          ...
          {Array.from({ length: totalSteps }, (_, index) => {
            ...
            return (
              <div key={stepNumber} className="flex flex-col items-center" style={{ width: '120px' }}>
                ...
                <div
                  className={`
                    flex items-center justify-center w-10 h-10 rounded-full border-2 transition-all duration-300 bg-gray-900
                    ${isCompleted ? "bg-green-600 border-green-600" : ""}
                    ${isCurrent ? "bg-blue-600 border-blue-600 ring-4 ring-blue-600/30" : ""}
                    ${!isCompleted && !isCurrent ? "bg-gray-700 border-gray-600" : ""}
                  `}
                >
                  {isCompleted ? (
